...
# update motd and list available instances to login
...
echo "Autoscaling group name: ${main_asg}"
# get all instances in ASG
for ip in `aws ec2 --region ${aws_region} describe-instances --filters Name=tag-key,Values='aws:autoscaling:groupName' Name=tag-value,Values=${main_asg} --output text --query 'Reservations[*].Instances[*].[PrivateIpAddress]'`; do
  echo $ip
done
echo ""
echo "========================================================================="
